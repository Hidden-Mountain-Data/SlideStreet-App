...
import type { Router, SalesDetails } from '@/@fake-db/types'
import axios from '@axios'

const search = ref('')
const productList = ref<Router[]>([])
...
onMounted(() => {
  axios.get('device-actions/devices').then(res => {
  ...
  <div>
    <VCardText>
      <VRow>
        <VCol cols="12" offset-md="8" md="4">
          ...
          <VTextField v-model="search" density="compact" label="Search" append-inner-icon="mdi-magnify" single-line
    ...
    <VDataTable :headers="headers" :items="productList" :search="search" :items-per-page="5">
      ...
      <template #item.product.category="{ item }">
        <div class="d-flex align-center">
          <VAvatar v-for="(category, index) in categoryIconFilter(item.raw.product.category)" :key="index" size="26"
      ...
      <template #item.buyer.name="{ item }">
        <div class="d-flex align-center">
          <VAvatar size="1.875rem" :color="!item.raw.avatar ? 'primary' : undefined"
      ...
      <template #item.status="{ item }">
        <VChip :color="resolveStatusColor(item.raw.payment.status)"
          :class="`text-${resolveStatusColor(item.raw.payment.status)}`" size="small" class="font-weight-medium">
